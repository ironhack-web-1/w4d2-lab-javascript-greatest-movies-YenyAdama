...
function getAllDirectors(y) {
    return y.map(movie=>movie.director)
}

//movies.forEach(e=>console.log(e.director));
//iteration 1.1
let directors=getAllDirectors(movies);
//method 1
//let directors_u = directors.filter((e, i) => {
  //  return directors.indexOf(e) === i;
//})
//method 2
let x = [1,2,3,3,5,6,7,5]
uniq = [...new Set(x)];

//console.log(directors_u);
...
function howManyMovies(moviesArray) {
    ...
    let stv_spl = moviesArray.filter(movie => movie.director==="Steven Spielberg")
    let drama_stv = stv_spl.filter(move=>move.genre.includes("Drama")).length
    console.log(drama_stv);
}
...
function scoresAverage(moviesArray) {
    if(moviesArray===0){
      ...
      const m_sc=moviesArray.filter(movie=>movie.score==0)
      console.log(m_sc);
...
let year_array=moviesArray.map(order=>order.year)
console.log(year_array);
let order_year=year_array.sort();
let y = moviesArray.sort((a,b) => {
  if(a.year < b.year){
    return -1
  }
})
